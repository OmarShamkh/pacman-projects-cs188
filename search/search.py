...
def depthFirstSearch(problem):
    ...
    while not path.isEmpty():
        (cur_state , Result) = path.pop()
        if problem.isGoalState(cur_state):
            return Result
        if cur_state not in visited:
            visited.append(cur_state)
            for successor,action,cost in problem.getSuccessors(cur_state):
                new_successor = (successor , Result+[action])
                path.push(new_successor)
...
def uniformCostSearch(problem):
    ...
    while not path.isEmpty():
        (cur_state , Result , cur_cost) = path.pop()
        if problem.isGoalState(cur_state):
            return Result
        if cur_state not in visited:
            visited.append(cur_state)
            for successor, action,stepcost in problem.getSuccessors(cur_state):
                ...
                path.push(new_successor , new_cost)
